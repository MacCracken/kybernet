...
asserts 55 properties across five passes — the boot sequence, the reactor
...
therefore **not** a cross-check, and this paragraph used to claim it was: the
fixture came from `scripts/mkcred.sh` (OpenSSL), which made the pass a genuine
interop test between two implementations. That dependency was removed at 1.6.1
because `openssl kdf ... ARGON2ID` needs OpenSSL 3.2 and CI runners ship 3.0,
so the gate was failing for a property of the runner rather than of kybernet.
The interop check did not disappear with it — it moved to `src/test.cyr`, which
verifies fixed OpenSSL-generated vectors. `scripts/mkcred.sh` remains the
operator tool, and CI still exercises its parameter validation.
